<?php if (session()->getFlashdata('error')): ?><div class="fixed top-4 right-4 z-50 animate-fade-in-down">
...
<?php endif; ?><!DOCTYPE html><html lang="id" class="h-full"><head>
    <meta charset="UTF-8">
...
</head><body class="h-full bg-gray-50">
                ...
                <div class="max-w-7xl mx-auto">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                        <div class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                            ...
                            <button onclick="document.getElementById('modal-pengumuman').classList.remove('hidden')" class="flex items-center gap-2 bg-primary-600 hover:bg-primary-700 text-white px-4 py-2.5 rounded-lg transition-colors shadow-sm hover:shadow-md">
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tujuan Desa</th>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                <?php foreach ($announcements as $i => $item): ?>
                                    <tr class="hover:bg-gray-50 transition-colors">
                                        ...
                                        <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate"> <?= esc($item['isi']) ?> </td>
                                        <td class="px-6 py-4 text-sm text-gray-500"> <?= $item['tujuan_desa'] ? esc($item['tujuan_desa']) : '<span class="italic text-gray-400">Semua Desa</span>' ?> </td>
                                        <td class="px-6 py-4 text-sm text-gray-500"> <?= date('d M Y H:i', strtotime($item['created_at'])) ?> </td>
                                        <td class="px-6 py-4 text-right text-sm space-x-3">
                                            <a href="#" onclick="editPengumuman(<?= htmlspecialchars(json_encode($item)) ?>)" class="text-blue-600 hover:text-blue-800 inline-flex items-center">
                                                <i class="fas fa-edit mr-1"></i><span>Edit</span>
                                            </a>
                                            <a href="<?= site_url('/dashboard/pengumuman/delete/' . $item['id']) ?>" class="text-red-600 hover:text-red-800 inline-flex items-center" onclick="return confirm('Yakin ingin menghapus pengumuman ini?')">
                                ...
                                <?php if (empty($announcements)): ?>
                                    <tr><td colspan="6" class="px-6 py-8 text-center text-gray-400">Belum ada pengumuman</td></tr>
                                <?php endif; ?>
                            </tbody>
                        </table>
                    </div>
        ...
        <div class="bg-white rounded-lg shadow-xl w-full max-w-lg p-6">
            ...
            <form id="formPengumuman" action="<?= site_url('dashboard/pengumuman_list/store') ?>" method="post">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="judul" id="judul" required class="w-full mt-1 border rounded px-3 py-2">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">Isi</label>
                    <textarea name="isi" id="isi" rows="4" required class="w-full mt-1 border rounded px-3 py-2"></textarea>
                </div>
                <div class="mb-4">
                ...
                <div class="flex justify-end space-x-3">
                    <button type="button" onclick="closeModal()" class="px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded">Batal</button>
                    <button type="submit" class="px-4 py-2 bg-primary-600 text-white hover:bg-primary-700 rounded">Simpan</button>
                </div>
            </form>
        </div>
    </div>
