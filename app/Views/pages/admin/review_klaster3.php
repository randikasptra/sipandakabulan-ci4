...
<head>
    ...
    <title><?= esc($title ?? 'Review Klaster 3') ?></title>
...
<body class="bg-gray-50 text-gray-800 antialiased  ml-64">
                ...
                <div class="max-w-5xl mx-auto">
                    <div class="mb-8">
                        <div class="flex items-center justify-between">
                            <div>
                                <h1 class="text-2xl font-bold text-gray-900">Review Data Klaster 3</h1>
                                <p class="mt-2 text-gray-600">Review dan verifikasi data indikator pada klaster 3</p>
                            </div>
                            <div>
                                <span
                                    class="badge <?= $klaster3['status'] === 'pending' ? 'bg-yellow-100 text-yellow-800' : ($klaster3['status'] === 'approved' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800') ?>">
                                    <?= ucfirst($klaster3['status']) ?>
                                </span>
                            </div>
                        </div>
                    ...
                    <?php
                    $fields = [
                        'kematianBayi' => 'Kematian Bayi',
                        'giziBalita' => 'Gizi Balita',
                        'asiEksklusif' => 'ASI Eksklusif',
                        'pojokAsi' => 'Pojok ASI',
                        'pusatKespro' => 'Pusat Kesehatan Reproduksi Remaja',
                        'imunisasiAnak' => 'Imunisasi Anak',
                        'layananAnakMiskin' => 'Layanan Anak Miskin',
                        'kawasanTanpaRokok' => 'Kawasan Tanpa Rokok',
                    ];
                    ?>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg">
                                ...
                                <p class="text-xl font-semibold text-gray-800">
                                    <?= esc($klaster3['tahun']) ?> / <?= bulanIndo($klaster3['bulan']) ?>
    ...
    <p class="text-lg font-medium <?= $klaster3['status'] === 'pending' ? 'text-yellow-600' : ($klaster3['status'] === 'approved' ? 'text-green-600' : 'text-red-600') ?>">
        <?= $klaster3['status'] === 'pending' 
            ? 'Menunggu' 
            : ($klaster3['status'] === 'approved' 
                ? 'Disetujui' 
                : 'Ditolak') ?>
    </p>
</div>
                   ...
                   <?php if ($klaster3['status'] === 'rejected'): ?>
                        ...
                        <form action="<?= base_url('dashboard/admin/klaster3/delete') ?>" method="post" class="mt-8">
                            <?= csrf_field() ?>
                            <input type="hidden" name="user_id" value="<?= $user_id ?>">
                            <input type="hidden" name="tahun" value="<?= esc($klaster3['tahun']) ?>">
                            <input type="hidden" name="bulan" value="<?= esc($klaster3['bulan']) ?>">
                    ...
                    <?php else: ?>
                        ...
                        <?php if ($klaster3['status'] === 'approved'): ?>
                            <div class="bg-green-100 text-green-800 px-4 py-3 rounded mb-6">
                                <i class="fas fa-check-circle mr-2"></i> Data ini telah <strong>diverifikasi</strong> dan disetujui.
                            </div>
                             <form action="<?= base_url('dashboard/admin/delete-approve-klaster3') ?>" method="post" class="mt-4">
                                    <?= csrf_field() ?>
                                    <input type="hidden" name="user_id" value="<?= $user_id ?>">
                        ...
                        <?php if ($klaster3['status'] === 'pending'): ?>
                            <form method="post" action="<?= base_url('dashboard/admin/klaster3/approve') ?>" class="mt-8">
                                <?= csrf_field() ?>
                                <input type="hidden" name="user_id" value="<?= $user_id ?>">
                                <input type="hidden" name="klaster" value="klaster3">
                                <input type="hidden" name="tahun" value="<?= esc($klaster3['tahun']) ?>">
                                <input type="hidden" name="bulan" value="<?= esc($klaster3['bulan']) ?>">
                                <input type="hidden" name="total_nilai" value="<?= esc($klaster3['total_nilai']) ?>">
